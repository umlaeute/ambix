...
# 
# Makefile to make pd-external AX // creates from template
#
# this is a copy of the svn-repository .../trunk/externals/template/
# https://pure-data.svn.sourceforge.net/svnroot/pure-data/trunk/externals/template/
# a local version (bzr revisioned) is in ~/src/pd/trunk/template
# please check (svn & bzr) for differences before releasing
#

#
# 2010-12-21	kr		created
...
# ax.pd -> for inclusion of ax-help.pd
PDOBJECTS = ax.pd
...
ifeq ($(UNAME),Darwin)
  ...
  ifeq ($(CPU),arm) # iPhone/iPod Touch
    ...
    EXTENSION = pd_darwin
    OS = iphoneos
	# TODO create PD_PATH dynamically
    PD_PATH = /Users/kr/Applications/Pd-0.42-6.app/Contents/Resources
...
# build universal 32-bit on 10.4 and 32/64 on newer
    ifeq ($(shell uname -r | sed 's|\([0-9][0-9]*\)\.[0-9][0-9]*\.[0-9][0-9]*|\1|'), 8)
	  ...
	  # TODO make universal instead of only i386 
      FAT_FLAGS = -arch i386
	  #FAT_FLAGS = -arch ppc -arch i386 -arch x86_64 -mmacosx-version-min=10.4
      SOURCES += $(SOURCES_iphoneos)
    endif
	# TODO create include-dir & lib-dir dynamically
    CFLAGS += $(FAT_FLAGS) -fPIC -I/usr/local/include
    LDFLAGS += $(FAT_FLAGS) -bundle -undefined dynamic_lookup -L/usr/local/lib
...
# install into ~/Library/Pd on Mac OS X since /usr/local isn't used much
	# TODO set pkglibdir dynamically
...
# _KR: somebody forgot this?
# TODO needs to be enhanced
install-exec:
	$(INSTALL_DATA) $(LIBRARY_NAME).$(EXTENSION) \
